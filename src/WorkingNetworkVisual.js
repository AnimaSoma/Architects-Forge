...
/**
 * WorkingNetworkVisual Component
 * 
 * A reliable visualization of multiple Observer Systems working together
 * to confirm observations and reach consensus about a Physical System.
 * 
 * This implementation uses separate timers for time tracking and simulation
 * updates to ensure reliable performance.
 */
...
const WorkingNetworkVisual = () => {
  // Simulation parameters with simplified defaults
  const [params, setParams] = useState({
    nodeCount: 5,                // Number of observer system nodes
    consensusThreshold: 0.6,     // Percentage needed for consensus
    updateInterval: 500,         // Ms between simulation updates
    connectionStrength: 0.7,     // How strongly nodes influence each other
    noiseLevel: 0.3              // Random variation in observations
  });

  // SEPARATE time tracking state - independent from simulation state
  const [elapsedTime, setElapsedTime] = useState(0);
  const startTimeRef = useRef(null);
  const lastTickRef = useRef(null);
  const animationFrameRef = useRef(null);

  // Simplified simulation state - separate from time tracking
  ...
  // Simulation interval reference - separate from animation frame
  const simulationIntervalRef = useRef(null);
  
  // Mounted ref to prevent state updates after unmount
  const isMountedRef = useRef(true);

  // Debug counter to track simulation ticks
  ...
  const updateSimulation = useCallback(() => {
    ...
    try {
      ...
      console.log(`Simulation tick #${tickCountRef.current}`);
      
      setSimState(prev => {
        // 1. Update physical system with simple random walk
  ...
  }, [params.noiseLevel, params.consensusThreshold]);

  // SEPARATE time update function using requestAnimationFrame
  const updateTimeDisplay = useCallback(() => {
    if (!isMountedRef.current || !simState.running) return;
  ...
  // Start simulation - with separate timers for simulation and display
  const startSimulation = useCallback(() => {
    ...
    // Start simulation updates on interval
    ...
    console.log(`Simulation interval set: ${simulationIntervalRef.current}, update every ${params.updateInterval}ms`);
  }, [updateSimulation, updateTimeDisplay, params.updateInterval]);
  ...
  // Format time display with two decimal places
      ...
      <div className="bg-gradient-to-r from-purple-900/60 to-indigo-900/60 backdrop-blur-sm rounded-lg p-8 text-center">
        ...
        <div className="mb-6 text-center">
          <div className="inline-block bg-black/30 px-6 py-3 rounded-lg">
            ...
            <div className={`text-3xl font-mono ${simState.running ? "text-green-400 animate-pulse" : "text-white"}`}>
        ...
        <div className={`mb-4 text-center ${simState.running ? 'visible' : 'invisible'}`}>
          <span className="inline-flex items-center px-3 py-1 rounded-full bg-green-500/20 text-green-400">
            ...
            Simulation Active - Tick #{tickCountRef.current}
          </span>
        </div>
                ...
                if (i < j) { // Only draw connection once between each pair
                  ...
                  return (
                    <line
                      ...
                      className={isActive && simState.running ? "animate-pulse" : ""}
                    />
                  );
                }
                return null;
...
export default WorkingNetworkVisual;
